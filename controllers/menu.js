 ...
 *  PERF: Limit the number of item in the homepage per section
**/
...
async function getAddons(req, res) {
    const food_item_id = req.params.item;
    result = await salesforce.conn.query(`
        SELECT Id, Description__c, Name__c, Price__c, Image_Url__c
        FROM Food_Item__c
        WHERE Id IN (SELECT Secondary_Food_Item__c FROM Addon__c WHERE Primary_Food_Item__c = '${food_item_id}')
    `);
